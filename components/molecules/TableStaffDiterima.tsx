...
export default function TableStaffDiterima() {
        ...
        const getCalonStaff = async () => {
            if (dinasName) {
                ...
                if (response && Array.isArray(response)) {
                    // Filter hanya calon staff dengan status "Diterima"
                    const filteredStaff = response.filter((staff: any) => staff.status === "Diterima");
                    setCalonStaff(filteredStaff);
                } else {
                    // Jika response undefined atau bukan array, set calonStaff ke array kosong
                    setCalonStaff([]);
                }
            }
        };
